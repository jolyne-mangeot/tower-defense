...
void Pointer_manager::instantiate_level_core(const int level_id) {
    this->level_core_ptr = make_shared<Level_core>(
...
void Pointer_manager::instantiate_user_data() {
    ...
    const weak_ptr<Level_core> level_weak_ptr = this->level_core_ptr;
    this->user_data_ptr->current_level_ptr = level_weak_ptr;
...
void Pointer_manager::instantiate_level_wave() {
    if (this->level_core_ptr->current_wave_id > 1) {
        this->current_wave_ptr->~Level_wave();
...
void Pointer_manager::instantiate_enemy_factories() {
    int factories_x = this->level_core_ptr->checkpoints_coordinates->at(0).at(0);
    int factories_y = this->level_core_ptr->checkpoints_coordinates->at(0).at(1);
...
//     towers_ptr.reserve(this->level_core_ptr->number_of_turrets);
//
//     for (int i = 0; i < this->level_core_ptr->number_of_turrets; i++) {
//
//         shared_ptr<Tower> tower_ptr = make_shared<Tower>(
//             this->level_core_ptr->turrets_coordinates.at(i).at(0),
//             this->level_core_ptr->turrets_coordinates.at(i).at(1));
//
...
void Pointer_manager::update_enemies_through() const {
    this->level_core_ptr->enemies_through++;
    if (this->level_core_ptr->enemies_through >= 3) {
        this->level_core_ptr->current_wave_pointer.lock()->~Level_wave();
    }
}
